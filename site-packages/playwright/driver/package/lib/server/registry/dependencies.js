...
// Modified: Added Fedora/RHEL/CentOS/Rocky/AlmaLinux distribution detection
...
async function installDependenciesLinux(targets, dryRun) {
   ...
   if (hasDnf) {
     ...
     updateCmd = ["dnf", "check-update"]; // dnf check-update returns non-zero if updates available, but that's OK
   } else if (hasApt) {
     // Debian/Ubuntu
     installCmd = ["apt-get", "install", "-y", "--no-install-recommends"];
     updateCmd = ["apt-get", "update"];
   } else {
     throw new Error("Neither apt-get nor dnf found. Cannot install dependencies.");
   }
   
// For Fedora/RHEL, show what packages to install instead of trying to install Ubuntu packages
    const isDnfSystem = hasDnf && !hasApt;
    
    if (isDnfSystem) {
      console.log(`\n=== Fedora/RHEL detected ===`);
      console.log(`The following packages need to be installed for browser support:`);
      console.log(`\n  dnf install -y ${uniqueLibraries.join(" ")}`);
      console.log(`\nNote: Ubuntu package names may not match Fedora packages exactly.`);
      console.log(`You may need to install equivalent Fedora packages manually.\n`);
      
      if (!dryRun) {
        console.log(`Skipping automatic installation on Fedora/RHEL.`);
        console.log(`Please install the required packages manually or use: sudo dnf install <package>`);
      }
      return;
    }
    ...
    commands.push([...installCmd, ...uniqueLibraries].join(" "));
...
async function commandExists(command) {
   try {
     const { code } = await (0, import_spawnAsync.spawnAsync)("which", [command]);
     return code === 0;
   } catch (e) {
     return false;
   }
...
async function validateDependenciesLinux(sdkLanguage, linuxLddDirectories, dlOpenLibraries) {
    ...
    const missingDLOPENLibs = await missingDLOPENLibraries(dlOpenLibraries);
    for (const dep of missingDLOPENLibs)
      missingDeps.add(dep);
    if (!missingDeps.size)
      return;
    ...
    if (dockerInfo && !dockerInfo.driverVersion.startsWith((0, import_userAgent.getPlaywrightVersion)(
      ...
      // Add appropriate package manager instructions
      const hasDnf = await commandExists("dnf");
      const hasApt = await commandExists("apt-get");
      if (hasDnf) {
        errorLines.push(`    ${maybeSudo}dnf install ${[...missingPackages].join(" ")}`);
      } else if (hasApt) {
        errorLines.push(`    ${maybeSudo}apt-get install ${[...missingPackages].join("\\\n        ")}`);
      } else {
        errorLines.push(`    Please install dependencies using your system's package manager`);
      }
